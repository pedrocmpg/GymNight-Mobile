...
- [ ] 1. Implement Env_Config module
  - [ ] 1.1 Create `src/config/env.ts` with `EnvConfig`, `EnvValidationResult`, and `validateEnvConfig`
  ...
  - [ ]* 1.2 Write property test for Env_Config validation
  ...
  - [ ]* 1.4 Write structural/grep-based tests for secret hygiene
...
- [ ] 2. Implement Supabase_Client construction
  - [ ] 2.1 Create `src/auth/supabaseClient.ts` with `createSupabaseClient(config: EnvConfig): SupabaseClient`
    - Single call site for `createClient` from `@supabase/supabase-js`
    - _Requirements: 2.3, 3.1_

  - [ ]* 2.2 Write structural test for single `createClient`/Env_Config read-site enforcement
...
- [ ] 3. Implement SupabaseAuthClient adapter
  - [ ] 3.1 Create `src/auth/supabaseAuthClientAdapter.ts` with `createSupabaseAuthClientAdapter(client): SupabaseAuthClient`
  ...
  - [ ]* 3.2 Write property test for successful sign-in field mapping
  ...
  - [ ]* 3.3 Write property test for error message mapping
  ...
  - [ ]* 3.4 Write property test for never propagating uncaught exceptions
...
- [ ] 4. Implement Token_Validator (JWT expiration check)
  - [ ] 4.1 Create `src/auth/jwtTokenValidator.ts` with `decodeJwtPayload` (base64url + JSON.parse, no library) and `jwtTokenValidator: TokenValidator`
    - `isExpired` returns `true` for undecodable tokens, tokens missing a numeric `exp`, or `exp` <= current time (seconds)
    - _Requirements: 4.1, 4.2_

  - [ ]* 4.2 Write property test for token expiration logic
...
- [ ] 5. Implement Session_Refresher adapter
  - [ ] 5.1 Create `src/auth/supabaseSessionRefresher.ts` with `createSupabaseSessionRefresher(client): SessionRefresher`
  ...
  - [ ]* 5.2 Write property test for successful refresh field mapping
  ...
  - [ ]* 5.3 Write property test for failed refresh never persisting
...
- [ ] 6. Implement Session_Store
  - [ ] 6.1 Create `src/auth/sessionStore.ts` with `SessionStore` interface and `createSessionStore()` (in-memory `set`/`clear`/`getCurrentSession`)
    - `getCurrentSession` satisfies `AuthInterceptor`'s `SessionProvider` contract
    - _Requirements: 10.2_

  - [ ] 6.2 Wire session-producing call sites to `sessionStore.set`/`clear`
  ...
  - [ ]* 6.3 Write property test for successful refresh propagation to storage and provider
  ...
  - [ ]* 6.4 Write property test for SessionProvider reflecting the latest session event
...
- [ ] 7. Implement logout port adapters
  - [ ] 7.1 Create `src/auth/logoutAdapters.ts`: `createSupabaseLogoutPort(client): SupabaseLogoutPort`
    - Call `client.auth.signOut()`; throw on declared error; let thrown/rejected failures propagate unchanged
    - _Requirements: 7.1_

  - [ ] 7.2 Add `createLogoutStoragePort(sessionStore): LogoutStoragePort` to `src/auth/logoutAdapters.ts`
  ...
  - [ ] 7.3 Add `createLogoutWipePort(db): LogoutWipePort` to `src/auth/logoutAdapters.ts`
  ...
  - [ ] 7.4 Wire the three logout port adapters into `Logout_Manager`'s constructor (Bootstrap_Sequence wiring point, implemented fully in task 12)
    - _Requirements: 7.4_

  - [ ]* 7.5 Write property test for SupabaseLogoutPort failure propagation
  ...
  - [ ]* 7.6 Write property test for LogoutStoragePort failure propagation
  ...
  - [ ]* 7.7 Write property test for LogoutWipePort failure propagation
...
- [ ] 8. Checkpoint - Ensure all auth-layer tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. Implement Sync_Cycle_Runner HTTP layer and push branch
  - [ ] 9.1 Create `src/sync/syncCycleRunner.ts` with `SyncHttpClient`, `HttpResult` types and the `createSyncCycleRunner(deps)` composition function
    - Read `backend_base_url` from `EnvConfig` only at construction time (single call site)
    - _Requirements: 2.4, 9.1_

  - [ ] 9.2 Implement `runPushBranch` per the design's push outcome table
  ...
  - [ ]* 9.3 Write property test for push branch outcome totality
...
- [ ] 10. Implement Sync_Cycle_Runner pull branch
  - [ ] 10.1 Implement `runPullBranch` per the design's pull outcome table
  ...
  - [ ]* 10.2 Write property test for pull branch outcome totality
  ...
  - [ ]* 10.3 Write property test for single Authorization header attachment at dispatch time
  ...
  - [ ]* 10.4 Write structural test for single Backend_Base_URL read site
...
- [ ] 11. Checkpoint - Ensure all sync-layer tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 12. Implement App_Navigator
  - [ ] 12.1 Create `src/navigation/AppNavigator.tsx` with `RootStackParamList` and the `AppNavigator` component
  ...
  - [ ]* 12.2 Write property test for bootstrap navigation routing totality
  ...
  - [ ]* 12.3 Write property test for Auth_Screen unreachability while a session is present
  ...
  - [ ]* 12.4 Write structural test for unique, non-duplicate route names
    - Assert exactly four uniquely named routes with no duplicates
    - _Requirements: 5.2_

- [ ] 13. Add the two Dashboard callback props (Criterion 14.5 exception)
  - [ ] 13.1 Extend `DashboardScreenProps` in `src/screens/DashboardScreen/DashboardScreen.tsx` with `onStartSession: () => void` and `onLogout: () => void` only
  ...
  - [ ]* 13.2 Write structural test confirming Dashboard_Screen's interface diff is limited to the two new callbacks
...
- [ ] 14. Implement Screen_Containers
  - [ ] 14.1 Create `src/navigation/containers/AuthScreenContainer.tsx`
  ...
  - [ ]* 14.2 Write property test for sign-in navigation routing totality
  ...
  - [ ] 14.3 Create `src/navigation/containers/DashboardScreenContainer.tsx`
  ...
  - [ ]* 14.4 Write property test for logout navigation/state-preservation totality
  ...
  - [ ]* 14.5 Write property test for concurrent logout trigger collapsing
  ...
  - [ ] 14.6 Create `src/navigation/containers/WorkoutCreatorScreenContainer.tsx`
  ...
  - [ ]* 14.7 Write property test for workout save navigation/error/data-preservation totality
  ...
  - [ ] 14.8 Create `src/navigation/containers/ActiveSessionScreenContainer.tsx`
  ...
  - [ ]* 14.9 Write per-container example tests asserting live (non-hardcoded) prop sourcing
  ...
  - [ ] 14.10 Wire the four Screen_Containers into `AppNavigator`'s `Stack.Screen` render props
    - _Requirements: 5.2, 5.3_

- [ ] 15. Checkpoint - Ensure all navigation-layer tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 16. Wire the Bootstrap_Sequence in App.tsx
  - [ ] 16.1 Replace `App.tsx`'s placeholder content with the full Bootstrap_Sequence
  ...
  - [ ] 16.2 Create the trivial `StartupErrorScreen` presentational component
    - Lists every offending variable name; renders in place of the navigator on invalid env config
    - _Requirements: 1.6_

  - [ ]* 16.3 Write one-shot wiring tests for the Bootstrap_Sequence
...
- [ ] 17. Implement backend migration for `users.email` nullability
  - [ ] 17.1 Create `gymnight/backend/alembic/versions/007_make_users_email_nullable.py`
  ...
  - [ ]* 17.2 Write a structural test on the migration script
    - Static inspection asserting only `nullable=True` is changed on `users.email` (no type/default/index change)
    - _Requirements: 12.1, 12.2_

  - [ ]* 17.3 Write backend integration tests against a migrated test database
